...
if JACC.backend != "metal" && JACC.backend != "amdgpu"
    @testset "CG Async" begin
        ...
        a1 = JACC.Async.fill(1, FloatType(4.0), SIZE)
        a2 = JACC.Async.ones(1, SIZE)
        r = JACC.Async.fill(2, FloatType(0.5), SIZE)
        p = JACC.Async.fill(1, FloatType(0.5), SIZE)
...
elseif JACC.backend == "metal"
    @skipped_testset "rand-Float64" "no Float64 on Metal"
else
    @skipped_testset "rand-Float64" "no custom RNG"
end

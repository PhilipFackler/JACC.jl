...
function JACC.parallel_reduce(
    ...
    @oneapi items=items groups=groups _parallel_reduce_oneapi(Val(items), N, op, ret, f, x...)
    @oneapi items=items groups=1 reduce_kernel_oneapi(Val(items), groups, op, ret, rret)
    oneAPI.synchronize()
...
function reduce_kernel_oneapi(
    ...
    if (i == 1)
        @inbounds shared_mem[i] = op(shared_mem[i], shared_mem[i + 1])
